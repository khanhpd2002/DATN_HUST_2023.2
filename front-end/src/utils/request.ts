...
export const chatService = service(import.meta.env.VITE_CHAT_API)
export const ecommerceService = service(import.meta.env.VITE_ECOMMERCE)
export const bookingService = service(import.meta.env.VITE_BOOKING)
export const trackingService = service(import.meta.env.VITE_TRACKING_SERVICE)
export const biddingService = service(import.meta.env.VITE_BIDDING_API)

//  axios.create({
//   baseURL: import.meta.env.GARAGE_SERVICE,
//   timeout: 15000
// })
biddingService.interceptors.request.use(
	(config) => {
		const token = localStorage.getItem('ecommerceAccessToken')
		if (token) {
			config.headers['Authorization'] = 'Bearer ' + token
		}
		config.headers['Content-Domain'] = import.meta.env.VITE_BIDDING_API
		return config
	},
	(error) => {
...
biddingService.interceptors.response.use(
	(response: any) => {
		/**
		 * response data
		 *   {
...
ecommerceService.interceptors.request.use(
	(config) => {
		const token = localStorage.getItem('ecommerceAccessToken')
		if (token) {
			config.headers['Authorization'] = 'Bearer ' + token
		}
		config.headers['Content-Domain'] = import.meta.env.VITE_ECOM_DOMAIN
		return config
	},
	(error) => {
		return Promise.reject(error)
...
garageService.interceptors.request.use(
	(request: any) => {
		...
		const garageTokenType = localStorage.getItem('garage_token_type')

		if (garageToken) {
			request.headers['Authorization'] = `${garageTokenType} ${garageToken}`
		}
		return request
		// request.headers["Access-Control-Allow-Origin"] = "*";
		// request.headers["Access-Control-Allow-Methods"] = "HEAD, GET, POST, PUT, PATCH, DELETE";
...
// respone拦截器
garageService.interceptors.response.use(
	(response: any) => {
		/**
		 * response data
		 *   {
...
function extractFileNameFromContentDispositionHeader(value: any) {
	const patterns = [
		/filename\*=[^']+'\w*'"([^"]+)";?/i,
		/filename\*=[^']+'\w*'([^;]+);?/i,
		/filename="([^;]*);?"/i,
	...
	let responseFilename = [] as any[]
	patterns.some((regex) => {
		responseFilename = regex.exec(value) as any[]
...
export function useResHeadersAPI(headers: any, resData: any) {
	...
	if (disposition) {
		...
		filename = extractFileNameFromContentDispositionHeader(disposition)
			? extractFileNameFromContentDispositionHeader(disposition)
			: ''
