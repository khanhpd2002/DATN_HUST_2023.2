...
import vn.com.cardoctor.garage_service.repositories.garages.CarSubSystemRepositoryCustom;
...
public class CarSubSystemRepositoryImpl implements CarSubSystemRepositoryCustom {
    @PersistenceContext
    ...
    public PagingDataResponse findAllCarSubSystem(String name, String description, Integer status,
        ...
        jpql.append("select * from car_sub_systems css ");
        jpql.append("where 1 = 1 ");

        if (Objects.nonNull(name)) {
            params.put("name", "%" + name + "%");
            jpql.append("and css.name like :name ");
        }
        if (Objects.nonNull(description)) {
            params.put("description", "%" + description + "%");
            jpql.append("and css.description like :description ");
        }
        if (Objects.nonNull(status)) {
            params.put("status", status);
            jpql.append("and css.status = :status ");
        }
        ...
        jpql.append(" order by css.created_at desc limit :limit offset :offset ");
        ...
        Query q = entityManager.createNativeQuery(jpql.toString(), CarSubSystem.class);
